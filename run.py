...
def cmd_resume(decision: str = "", note: str = ""):
    ...
    if gate == "plan_approval":
        if approved:
            ...
            doc0.write_text(
                text.replace(
                    "shared_plan_approved: false", "shared_plan_approved: true"
                )
            )
    ...
    ps.add_log(
        "info" if approved else "warn", f"Gate {gate} resolved: {decision}", note
    )
    state_store.save(ps, ROOT)
...
def cmd_status():
    ...
    for mid in sorted(milestones):
        ...
        for fid in sorted(milestones[mid]):
            ...
            err = (
                c(RED, f" {fr.last_error[:50]}")
                if fr.last_error and fr.status.value == "failed"
                else ""
            )
...
def cmd_memory(feature_id: str = ""):
    ...
    if feature_id:
        ...
        total = sum(
            len([e for e in mem.get(s, []) if "_comment" not in e]) for s in sections
        )
...
def cmd_gh_check(feature_id: str) -> None:
    ...
    if not prs:
        ...
        print(
            c(
                DIM,
                "  The worker may not have opened it yet, or the title format is wrong.",
            )
        )
        sys.exit(1)
    ...
    print(c(GREEN if outcome == "approved" else RED, f"  human_gate set to: {outcome}"))
    ...
    if outcome == "approved":
        ...
        print(
            c(
                DIM,
                "  Address the review comments, push to the same branch, then re-check.",
            )
        )
    print()
...
def cmd_retry(feature_id: str, force: bool = False) -> None:
    ...
    if fr.retry_count >= MAX_FEATURE_RETRIES and not force:
        ...
        print(
            c(
                DIM,
                "  This usually means the doc3 spec or doc2 acceptance criteria "
    ...
    worker_results = {
        k: v for k, v in (current.get("worker_results", {}) or {}).items() if k != fid
    }
    git_results = {
        k: v for k, v in (current.get("git_results", {}) or {}).items() if k != fid
    }
    validator_results = {
        k: v
        for k, v in (current.get("validator_results", {}) or {}).items()
        if k != fid
    }
    merge_results = {
        k: v for k, v in (current.get("merge_results", {}) or {}).items() if k != fid
    }
    ...
    fr.validator_result = ""
    ps.add_log(
        "info", f"Retrying {fid} (attempt {fr.retry_count + 1})", retry_note[:200]
    )
    state_store.save(ps, ROOT)
