...
class TestModuleStructure:
    ...
    def test_secrets_manager_module_exists(self):
        assert os.path.isfile(os.path.join(MODULES_DIR, "secrets_manager", "main.tf")), \
...
class TestSecretsManagerSecurity:
    ...
    def test_db_url_secret_created(self):
        assert "DB_URL" in self.main, \
            "Secrets Manager must create DB_URL secret"

    def test_redis_url_secret_created(self):
        assert "REDIS_URL" in self.main, \
            "Secrets Manager must create REDIS_URL secret"

    def test_jwt_secret_created(self):
        assert "JWT_SECRET" in self.main, \
            "Secrets Manager must create JWT_SECRET"
    ...
    def test_initial_secret_values_are_placeholders(self):
        ...
        assert "REPLACE_AFTER_PROVISIONING" in self.main or \
               "placeholder" in self.main.lower(), \
    ...
    def test_ignore_changes_on_secret_values(self):
        ...
        assert "ignore_changes" in self.main and \
               "secret_string" in self.main, \
...
class TestTerraformBackend:
    ...
    def test_s3_backend_configured(self):
        assert 'backend "s3"' in self.backend, \
            "backend.tf must configure S3 backend"

    def test_encrypt_enabled_in_backend(self):
        assert "encrypt" in self.backend, \
            "backend.tf must reference encrypt option"
    ...
    def test_tfstate_files_gitignored(self):
        ...
        if os.path.exists(gitignore_path):
            ...
            assert "*.tfstate" in content, \
                ".gitignore must exclude *.tfstate files"
        else:
...
class TestRootModule:
    ...
    def test_vpc_module_called(self):
        assert 'module "vpc"' in self.main, \
            "Root main.tf must call the vpc module"

    def test_rds_module_called(self):
        assert 'module "rds"' in self.main, \
            "Root main.tf must call the rds module"
    ...
    def test_s3_module_called(self):
        assert 'module "s3"' in self.main, \
            "Root main.tf must call the s3 module"
    ...
    def test_environment_validation(self):
        ...
        assert '"dev"' in self.variables and \
               '"staging"' in self.variables and \
               '"prod"' in self.variables, \
    ...
    def test_rds_kms_key_output(self):
        ...
        assert "rds_kms_key_id" in self.outputs, \
            "outputs.tf must expose rds_kms_key_id"
    ...
    def test_secret_arns_in_outputs_not_values(self):
        ...
        assert "jwt_secret_arn" in self.outputs, \
            "outputs.tf must expose jwt_secret_arn"
            ...
            for line in lines:
                if "=" in line and not line.strip().startswith("#"):
                    ...
                    if value and len(value) > 40 and not value.startswith("<"):
                        pytest.fail(
                            f"infra/.env.example appears to contain a real secret value"
                        )
...
class TestVPCNetworkIsolation:
    ...
    def test_private_route_table_uses_nat(self):
        ...
        if private_rt_block:
            ...
            assert "internet_gateway" not in rt_text or \
                   "aws_internet_gateway" not in rt_text, \
