...
def filter_for_feature(
    ...
    return {
        "architecture_decisions": [
            e
            for e in mem.get("architecture_decisions", [])
            if real(e) and e.get("feature_id") in depends_chain
        ],
        "failed_approaches": [e for e in mem.get("failed_approaches", []) if real(e)],
        "discovered_constraints": [
            e
            for e in mem.get("discovered_constraints", [])
            if real(e)
            and (
...
def append_from_milestone(
    ...
    issues_m = re.search(
        r"## Issues discovered\s*```yaml\n(.*?)```", report_text, re.DOTALL
    )
    if not issues_m:
        return added
    for block in re.split(
        r"(?=^\s*- issue_id:)", issues_m.group(1), flags=re.MULTILINE
    ):
        block = block.strip()
            ...
            mem["failed_approaches"].append(
                {
                    ...
                    "what_was_tried": desc,
                    "why_it_failed": notes,
                    "do_not_retry": no_retry,
                    "alternative_used": notes if resolution == "workaround" else "",
                }
            )
        ...
        mem["open_risks"].append(
            {
                ...
                "timestamp": now,
                "severity": "high",
                "description": f"Security checklist not followed: {notes}",
                "mitigation_status": "open",
